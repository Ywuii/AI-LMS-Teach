...
function transformRoute(route: any) {
  const result: any = {
    path: route.path,
    name: route.name || route.path.replace(/\//g, "_"),
    meta: route.meta
  };

  if (route.component === "layout") {
    result.component = Layout;
  } else if (route.component === "chat_stream") {
    result.component = () =>
      import(`@/views/agent/qa/${route.component}.vue`);
  } else if (route.component === "chat") {
    result.component = () =>
      import(`@/views/agent/qa/${route.component}.vue`);
  }
  else {
    result.component = () =>
      import(`@/views/agent/${route.component}/index.vue`);
  }
...
function handleAsyncRoutes(routeList: any[]) {
  ...
  routeList.forEach(route => {
    const realRoute = transformRoute(route);
