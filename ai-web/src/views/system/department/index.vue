  ...
  <div class="maincontent">
    ...
    <div class="top">
      <el-form ref="formRef" :inline="true" :model="form" class="searchform">
        <el-form-item label="院系名称：" prop="name">
          <el-input v-model="form.name" placeholder="请输入院系名称" clearable class="!w-[180px]" />
        </el-form-item>
    ...
    <div ref="tableContainer" class="table">
      <el-table :data="dataList" style="width: 100%; margin-bottom: 20px" row-key="id" lazy :height="tableMaxHeight" default-expand-all>
        <el-table-column prop="id" label="院系ID" align="center" />
        ...
        <el-table-column prop="type" label="类型" align="center">
          <template #default="{ row }">
            <span v-if="row.type === 1">大学</span>
            <span v-else-if="row.type === 2">学院</span>
            <span v-else-if="row.type === 3">行政院系</span>
        ...
        <el-table-column label="操作" align="center" fixed="right" min-width="150px">
          ...
          <template #default="{ row }">
            <div class="ellink">
              ...
              <el-link type="primary" @click="handleCreat(row.id)">增加</el-link>
...
function handleDelete(row) {
  ElMessageBox.confirm("院系：" + row.name + " 将被永久删除，请谨慎操作！", "警告", {
    confirmButtonText: "确认删除",
    cancelButtonText: "取消",
    type: "warning"
  })
    .then(() => {
      console.log("删除院系", row);
      deleteDepart(row.id)
        .then(res => {
          onSearch();
