<template>
  <el-dialog v-model="dialogVisible" :title="props.isEditMode ? '编辑学生' : '创建学生'" :width="deviceDetection() ? '90%' : '50%'" @open="handleOpen" @close="handleCancel">
    <el-form ref="userForm" :rules="isEditMode ? rules_edit : rules_create" :model="userData" label-width="80px" label-position="left">
      ...
      <el-form-item label="所属院系" prop="dept">
        <el-cascader v-model="userData.dept" :options="deptData" :props="{ value: 'id', label: 'name', children: 'children', checkStrictly: true }" clearable filterable placeholder="无所属院系" />
      </el-form-item>
...
const userData = ref({}); // 创建一个可变的本地学生数据对象
const deptData = ref([]);
const rolesData = ref([]);
const userForm = ref();
...
const handleConfirm = () => {
  userForm.value.validate(valid => {
    emit("save", props.isEditMode ? "update" : "create", userData.value); // 向父组件发送保存事件并传递学生数据
...
const rules_create = {
  username: [
    ...
    { min: 2, max: 10, message: "学生名称长度限制2-10", trigger: "blur" }
  ],
