    ...
    <div class="right">
      ...
      <el-form ref="formRef" :inline="true" :model="form" class="searchform">
        <el-form-item label="学生名称：" prop="username">
          <el-input v-model="form.username" placeholder="请输入学生名称" clearable class="!w-[180px]" />
        </el-form-item>
      ...
      <div ref="tableContainer" class="table">
        <el-table :data="dataList" class="el-table" :height="tableMaxHeight" :show-overflow-tooltip="true">
          ...
          <el-table-column prop="dept_name" label="院系" align="center" />
          ...
          <el-table-column label="操作" align="center" fixed="right" min-width="150px">
            ...
            <template #default="{ row }">
              <div class="ellink">
                <el-link underline="never" type="primary" @click="handleEdit(row)">编辑</el-link>
                <el-link underline="never" type="danger" @click="handleDelete(row)">删除</el-link>
              </div>
            </template>
          </el-table-column>
...
const handleSave = (method, data) => {
  ...
  if (method === "create") {
    ...
    console.log("新增学生", method, newdata);
    postUser(newdata)
      .then(res => {
    ...
    const id = data.id;
    const newdata = userFilter(method, data);
    console.log("更新学生", method, newdata);
...
async function onSearch() {
  loading.value = true;
  // 获取学生数据，赋值dataList和分页
  await getUserList(form).then(res => {
    console.log(res.data);
    const students = res.data.filter(item => item.role_name[0] === "学生");
...
function handleDelete(row) {
  ElMessageBox.confirm("学生：" + row.username + " 将被永久删除，请谨慎操作！", "警告", {
