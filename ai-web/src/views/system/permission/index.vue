<template>
  <div class="maincontent">
        ...
        <el-table-column prop="name" label="名称" align="center" min-width="150px">
          <template #default="{ row }">
            <div style="display: flex; justify-content: center; align-items: center">
              <component v-if="row.meta.icon" :is="useRenderIcon(row.meta.icon)" style="min-width: 15px; min-height: 15px" />
              <span>{{ row.name }}</span>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" fixed="right" min-width="150px">
          ...
          <template #default="{ row }">
            <div class="ellink">
              ...
              <el-link v-if="row.menu_type === 1" type="primary" @click="handlePortPsermissionsCreat(row.id)">+权限</el-link>
    ...
    <!-- 菜单增删改查接口权限增加弹窗 -->
    <portpermissionadddialog v-model:visible="isPortDialogVisible" :parent="selectId" @savesuccess="onSearch()" />
  </div>
</template>
